...
const createProduct = async(req, res) =>{
    ...
    if(user.role == "user"){
       ...
       return res.status(400).json(new ApiErrors(
            "User Role requirnment didn't match."
        ))
    } 
...
try {
    ...
    const checkExistanceProduct = await Product.findOne({name:nameLowered})    
        ...
        return res.status(500).json(new ApiErrors(
            error.message || "Error in creating Product list at the method CreateProduct."
        ))
    }
}
...
// filter query
        ...
        const items = await Product.find(query).skip(skip).limit(limit).sort({price : "ascending" ? 1 : -1})
    
        if(!items){
            logger.error(`Product list is not found.`)
            res.status(500).json(new ApiErrors(
        ...
        return res.status(500).json(new ApiErrors(
           error.message || "Error in getting all products list."
        ))
    }
}
...
const getSingleProductAndUpdate = async(req, res) =>{
    ...
    if(user.role === "user"){
        ...
        return res.status(400).json(new ApiErrors(
            "User Role requirenmnt didn't match."
        ))
    }

    try {
        const NewData = req.body
        const product = await Product.findByIdAndUpdate(req.params.id,NewData,{new: true})

        if(!product){
            logger.error(`Product didn't found.`)
            return res.status(400).json(new ApiErrors(
                "Product details doesn,t found."
            ))
        }
        ...
        return res.status(500).json(new ApiErrors(
            error.message || "Error while updateing product."
        ))
    }
}
...
const getSingleProductAndDelete = async(req, res) => {
    ...
    try{
        ...
        return res.status(400).json(new ApiErrors(
             error.message || "Error in deleteProduct catch block."
        ));
    }
}

module.exports = {createProduct, getAllProduct,getSingleProductAndUpdate,getSingleProductAndDelete}