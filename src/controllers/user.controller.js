...
const createUser = async (req, res) => {
  try {
    ...
    if (!name || !email || !password) {
      ...
      return res.status(400).json(new ApiErrors("All Fields are required!"));
    }
    ...
    if (existingUser) {
      ...
      return res.status(400).json(new ApiErrors("User already exist."));
    }
    ...
    if (!user) {
      ...
      return res
        .status(400)
        .json(new ApiErrors("Error While User registration."));
    }
    ...
    return res
      .status(500)
      .json(new ApiErrors(error.message || "Create user Error."));
  }
};
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.status(400).json(new ApiErrors("Entered Email is invalid."));
    }
    ...
    if (!isMatched) {
      ...
      return res
        .status(400)
        .json(new ApiErrors("Entered Password is invalid."));
    }
    ...
    res.status(200).json(new ApiResponse("Token generated", token));
  } 
    ...
    return res
      .status(500)
      .json(new ApiErrors(error.message || `Error in login user method.`));
  }
};
...
const getUserProfile = async (req, res) => {
  try {
    ...
    if (!userData) {
      ...
      return res.status(400).json(new ApiErrors("User aren't logged in."));
    }

    logger.info(`User profile found.`);
    res.status(200).json(new ApiResponse("User Found.", user));
  } 
    ...
    return res
      .status(500)
      .json(new ApiErrors(error.message || "Error in getUserProfile method."));
  }
};
...
const logOut = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json(new ApiErrors(error.message || "Error in logout method."));
  }
};
